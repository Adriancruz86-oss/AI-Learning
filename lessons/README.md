# Lessons

This folder documents my learning progress from computer and machine-learning fundamentals into networking, cybersecurity, and 
cryptography.

## Artificial Intelligence and Machine Learning

1. [Lesson 001: Single Neuron](lesson_001_single_neuron.md)
2. [Lesson 002: Single Neuron Continued](lesson_002_single_neuron.md)
3. [Lesson 003: Multiple Examples](lesson_003_multiple_examples.md)
4. [Lesson 004: Gradient Descent](lesson_004_gradient_descent.md)
5. [Lesson 005: Multiple Inputs Explained](lesson_005_multiple_inputs_explained.md)

## Computer and Security Foundations

20. [Lesson 020: CPU and Machine-Code Flow](lesson_020_cpu_machine_code_flow.md)
21. [Lesson 021: Networking Fundamentals](lesson_021_networking_fundamentals.md)
22. [Lesson 022: Security Attack Lifecycle](lesson_022_security_attack_lifecycle.md)
23. [Lesson 023: Cryptography Fundamentals](lesson_023_cryptography_fundamentals.md)
24. [Lesson 024: Quantum-Safe Cryptography](lesson_024_quantum_safe_cryptography.md)
25. [Lesson 025: Processes, Services, Permissions, and Persistence](lesson_025_processes_services_permissions_persistence.md)
26. [Lesson 026: Latches, Flip-Flops, Registers, and CPU Timing](lesson_026_latches_flip_flops_registers.md)

## Learning Direction

The repository follows a bottom-up approach:

```text
Computer hardware and binary
        ↓
Python and programming logic
        ↓
Neurons and machine learning
        ↓
Networking and operating systems
        ↓
Cybersecurity and cryptography
        ↓
Security-focused projects
```

Each lesson records the concepts learned, practical examples, and connections to broader goals such as Security+, Network+, AI, 
and cybersecurity engineering.
